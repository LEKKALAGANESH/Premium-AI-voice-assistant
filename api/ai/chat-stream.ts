// Zero-Wait Engine: Edge Runtime Streaming Chat
// Vercel Edge for sub-100ms cold start + SSE streaming from Gemini
// Versatility Engine: Dynamic system prompt injection based on active mode
...
// === STRICT PERSONA ENGINE: Isolated Mode Prompts (Edge-side kernel) ===
// Synced with server/services/ai.ts — these are the hardened production prompts
...
const DEFAULT_PROMPT = MODE_PROMPTS.assistant;

function getSystemPrompt(mode?: string): string {
  if (!mode || !(mode in MODE_PROMPTS)) return DEFAULT_PROMPT;
  return MODE_PROMPTS[mode];
}

// Mode-specific generation config
...
export default async function handler(req: Request) {
  ...
  const { prompt, history, warmup, mode } = body;
  ...
  // Dynamic prompt + config based on active mode
  const systemPrompt = getSystemPrompt(mode);
  const genConfig = getGenerationConfig(mode);

  // Stream via ReadableStream (Edge-compatible)
      ...
      try {
        ...
        for await (const chunk of genStream) {
          ...
          if (text) {
            fullText += text;
            chunkCount++;
            send({ chunk: text, done: false, idx: chunkCount });
          }
        }

        send({ chunk: '', done: true, fullText, total: chunkCount });
      } catch (error) {
        send({
